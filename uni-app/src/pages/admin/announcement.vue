<template>
  <view class="container">
    <view class="toolbar">
      <button class="add-btn" @click="openAdd">+ 新增公告</button>
    </view>

    <view v-if="loadError" class="empty error">{{ loadError }}</view>
    <view v-if="list.length === 0 && !loadError" class="empty">暂无公告</view>
    ...
    <view class="list">
      <view class="item" v-for="item in list" :key="item.id">
        ...
        <text class="item-meta">{{ item.publisher }} · {{ item.publishDate }}</text>
    ...
    <view class="dialog-overlay" v-if="showDialog" @tap="onOverlayTap">
      <view class="dialog-box" @tap.stop>
        <text class="dialog-title">{{ isEdit ? '编辑公告' : '新增公告' }}</text>
        <textarea class="field" v-model="form.title" placeholder="标题" />
        <textarea class="field" v-model="form.category" placeholder="分类" />
        <textarea class="field" v-model="form.publisher" placeholder="发布人" />
        <textarea class="field textarea" v-model="form.content" placeholder="内容" />
...
const form = ref({ title: '', category: '', content: '', publisher: '' })
...
const openAdd = () => {
  ...
  form.value = { title: '', category: '', content: '', publisher: '' }
  showDialog.value = true
}
const openEdit = (item) => {
  isEdit.value = true
  editingId.value = item.id
  form.value = { title: item.title, category: item.category, content: item.content, publisher: item.publisher }
  showDialog.value = true
}
...
const handleSave = async () => {
  try {
    if (isEdit.value) {
      await api.announcement.update(editingId.value, form.value)
    } else {
      await api.announcement.add(form.value)
    }
...
const handleDelete = (item) => {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          await api.announcement.delete(item.id)
...
.item-meta { font-size: 11px; color: #aaa; display: block; margin-top: 6px; }
